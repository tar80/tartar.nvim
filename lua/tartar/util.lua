...
-- Repeats a given "pattern" a specified number of "count" times.
---@param pattern string The pattern to be repeated.
---@param count number? The number of times to repeat the pattern (can be a string).
---@return string - The repeated pattern, or an empty string if "count" is not a valid number.
function M.repeat_pattern(pattern, count)
  count = tonumber(count)
  local ret = ''
  if type(count) == 'number' then
    ret = pattern:rep(count)
  end
  return ret
end
